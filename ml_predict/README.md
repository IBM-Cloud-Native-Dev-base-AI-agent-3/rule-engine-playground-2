# 🤖 ml_predict 폴더 가이드: 머신러닝 예측 엔진 이해하기

이 폴더는 MySQL 데이터베이스에 등록된 규칙을 바탕으로 신청자의 정보를 분석하여, 실제 경쟁자들 사이에서 **"최종적으로 서류 심사를 통과해 입주할 수 있을지(합격 확률)"**를 예측하는 머신러닝 파이프라인이 들어있는 곳입니다.

이해하기 쉽게 그림과 비유를 통해 작동 원리를 설명해 드릴게요!
